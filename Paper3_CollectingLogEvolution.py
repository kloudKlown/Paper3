...
####### CHECKING WHAT LOG LEVEL IT IS
    ...
    if len(deletedLog) > 1:
        if tmp2[0] != tmp2Deleted[0]:
            ...
            if len(metricsNeeded.logLevelChangeFlag) < 5:
                metricsNeeded.logLevelChangeFlag = metricsNeeded.logLevelChangeFlag + tmp2Deleted[0] + '-->' + tmp2[0] 

            else:
                metricsNeeded.logLevelChangeFlag = metricsNeeded.logLevelChangeFlag + '-->' + tmp2[0]
            ...
            while 1:
                ...
                if re.match('^\+.*',list1[aes].logLine):
                    ...
                    if addedLog == list1[aes].logLine.lstrip('-|+').lstrip() or Levenshtein.ratio(addedLog,list1[aes].logLine.lstrip('-|+').lstrip()) > 0.9 :
                        ...
                        metricsNeeded.AddedCodeBlock = list1[aes].logLine + '--- THIS IS THE LOG LINE ---'
